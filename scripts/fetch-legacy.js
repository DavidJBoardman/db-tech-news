// SIGNAL — LEGACY pipeline (the pre-Sept-2026 original), kept ONLY to power the
// "Old" side of the in-app Old|New trial toggle. This is a verbatim copy of the
// original scripts/fetch.js: AI-only sources, original ranking (0.35 recency /
// 18h half-life, no burst / topic-priority), no noise filter, no routing.
// It writes data/items-legacy.json + data/digest-legacy.json. Do not add features
// here — it exists to preserve the old behaviour for comparison. Delete both
// -legacy scripts (and the toggle) to end the trial. Run: node scripts/fetch-legacy.js
//
// The live pipeline is scripts/fetch.js.
import { createHash } from 'node:crypto';
import { writeFile, mkdir } from 'node:fs/promises';
import { SOURCES, TOPIC_RULES, ORG_RULES, BADGE_RULES } from './sources-legacy.js';
...
async function main() {
  ...
  await writeFile(new URL('../data/items-legacy.json', import.meta.url), JSON.stringify({ generated, items }, null, 1) + '\n');
  await writeFile(new URL('../data/digest-legacy.json', import.meta.url), JSON.stringify({ generated, digest }, null, 1) + '\n');
